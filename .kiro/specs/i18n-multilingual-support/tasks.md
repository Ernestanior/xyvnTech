...
- [ ] 7. 集成语言切换器到导航栏
  - [x] 7.1 更新 NavbarEnhanced 组件
    - 导入并添加 LanguageSwitcher 组件
    - 调整布局以适应语言切换器
    - 确保桌面和移动端都正确显示
    - _需求：4.1, 4.4_
  
  - [x] 7.2 更新导航链接使用 i18n 路由
    - 使用 next-intl 的 Link 组件
    - 确保所有链接包含语言前缀
    - _需求：2.2, 2.3_

- [ ] 8. 实现翻译工具函数
  - [x] 8.1 创建 lib/i18n/utils.ts
    - 实现日期格式化函数
    - 实现数字格式化函数
    - 实现货币格式化函数
    - 实现翻译回退函数
...
- [x] 16. 实现 SEO 优化功能
  - [x] 16.1 生成 hreflang 标签
    - 在页面元数据中添加 hreflang 标签
    - 确保所有语言版本都被链接
    - _需求：5.1_
  
  - [x] 16.2 生成多语言 sitemap
    - 创建 sitemap 生成函数
    - 为每种语言生成独立的 sitemap
    - _需求：5.2_
  
  - [x] 16.3 添加 Open Graph 和 Twitter Card 元数据
    - 为每种语言生成本地化的社交媒体元数据
    - _需求：5.5_
